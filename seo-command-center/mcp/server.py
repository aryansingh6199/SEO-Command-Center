...
def seo_fix() -> dict:
    """Run the fixer logic to generate titles and redirect maps."""
    rows = RUN.get("rows", [])
    issues = RUN.get("issues", [])

    # Generate fixes
    redirects = fixer.generate_redirect_map(rows)
    content = fixer.generate_content_fixes(rows, issues)
    ...
    return {"titles_fixed": len(content["titles"]), "redirects_fixed": len(redirects)}
